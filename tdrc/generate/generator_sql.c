...
{
	generator_sql_t *self = TDR_CONTAINER_OF(super, generator_sql_t, super);
	TDR_UNUSED(yylloc);
...
static error_code_t on_field(generator_t *super, const YYLTYPE *yylloc, const syn_field_t *field)
{
	...
	{
	case E_ST_BOOL:
		generator_print(super, 1, "boolean");
		break;
	case E_ST_INT8:
	...
	case E_ST_STRING:
		{
			...
			if(ui64 <= TDR_UINT8_MAX)
			{
				generator_print(super, 1, "char(%"PRIu64")", ui64);
			}
			else if(ui64 <= TDR_UINT16_MAX)
			{
