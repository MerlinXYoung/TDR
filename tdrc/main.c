...
static generator_sql_t tdr_sql_generator;
static generator_cs_t tdr_cs_generator;
		...
		case 'g':
			{
				...
				{
					generator_cs_init(&tdr_cs_generator, &g_parser.symbols);
					generator = &tdr_cs_generator.super;
				}
				else if(strcmp(arg, "sql") == 0)
				{
					generator_sql_init(&tdr_sql_generator, &g_parser.symbols);
					generator = &tdr_sql_generator.super;
				}			
			}
			break;
		default:
