#ifndef _H_TDR_UNZIP_H
#define _H_TDR_UNZIP_H
...
typedef struct _tdr_unzip_global_info
{
    uint16_t number_entry;         /* total number of entries in
                                     the central dir on this disk */
    uint16_t size_comment;         /* size of the global comment of the zipfile */
}tdr_unzip_global_info;

typedef struct _tdr_unzip_file_info_s
{
...
}tdr_unzip_file_info_s;

typedef struct _tdr_unzip_file_info_internal_s
{
    uint32_t offset_curfile;/* relative offset of local header 8 bytes */
}tdr_unzip_file_info_internal_s;

typedef struct _tdr_unzip_read_info_s
{
    ...
    int   raw;
}tdr_unzip_read_info_s;

typedef struct _tdr_unzip_s
{
    FILE* filestream;        /* io structore of the zipfile */


    tdr_unzip_global_info gi;       /* public global information */
    uint32_t byte_before_the_zipfile;/* byte before the zipfile, (>0 for sfx)*/
    ...
    tdr_unzip_file_info_s cur_file_info; /* public info about the current file in zip*/
    tdr_unzip_file_info_internal_s cur_file_info_internal; /* private info about it*/
    tdr_unzip_read_info_s pfile_in_zip_read; /* structure about the current
                                        file if we are decompressing it */
}tdr_unzip_s;

tdr_error_code_t tdr_unzip_init(tdr_unzip_s *self, const void *path);

tdr_error_code_t tdr_unzip_locate(tdr_unzip_s *self, const char *szFileName);
